...
def add_cat_seg_config(cfg):
    ...
    cfg.MODEL.HYPERSPHERICAL = CN()
    
    # HPA: Hyperspherical Prototype Alignment
    # 在超球面上维护类别原型，使用测地线距离进行对齐
    cfg.MODEL.HYPERSPHERICAL.USE_HPA = True
    cfg.MODEL.HYPERSPHERICAL.HPA_MOMENTUM = 0.99      # 原型动量更新系数
    cfg.MODEL.HYPERSPHERICAL.HPA_TEMPERATURE = 0.07  # 基础温度参数
    
    # AFR: Adaptive Feature Rectification
    # 自适应特征校正，抑制有害迁移特征
    cfg.MODEL.HYPERSPHERICAL.USE_AFR = True
    cfg.MODEL.HYPERSPHERICAL.AFR_REDUCTION = 16      # 通道压缩比例
    
    # Loss weight for innovation auxiliary losses
    cfg.MODEL.HYPERSPHERICAL.LOSS_WEIGHT = 0.1